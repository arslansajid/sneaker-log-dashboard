...
class Sidebar extends Component {
  ...
  render() {
    ...
    const navLink = (item, key, classes) => {
      ...
      const adminOrSuperAdmin = props.user &&
        ((props.user.department  &&
        props.user.department === 'admin') || !!props.user.superadmin);
      return (
            ...
            : item.requiredRole && item.requiredDepartment ?
              <HasRole requiredRole={item.requiredRole} requiredDepartment={item.requiredDepartment}>
                <NavLink to={url} className={classes.link} activeClassName="active" onClick={this.hideMobile}
                         exact={true}>
                  {item.icon ? <i className={classes.icon}></i> : null}
                  {adminOrSuperAdmin && item.alternateName ? item.alternateName : item.name}{badge(item.badge)}
                </NavLink>
              </HasRole>
              :
              <NavLink to={url} className={classes.link} activeClassName="active" onClick={this.hideMobile}
    ...
    const navDropdown = (item, key) => {
      return (
        item.requiredRole && item.requiredDepartment ?
          <HasRole requiredRole={item.requiredRole} requiredDepartment={item.requiredDepartment} key={key}>
            <li className={this.activeRoute(item.url, props)}>
